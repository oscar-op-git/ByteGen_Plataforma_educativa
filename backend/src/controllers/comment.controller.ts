...
// Si quieres, puedes mover esto a un util global
...
export async function postMainComment(req: Request, res: Response) {
  ...
  try {
    const comment = await createMainCommentForPlantilla(
      plantillaId,
      authorName,
      content.trim()
    );
    ...
    if (err instanceof MainCommentExistsError) {
      return res.status(409).json({
        message:
          "Ya existe un comentario principal. Solo se permiten respuestas.",
      });
    }
...
export async function postReply(req: Request, res: Response) {
  ...
  try {
    const reply = await createReplyForComment(
      commentId,
      authorName,
      content.trim()
    );
